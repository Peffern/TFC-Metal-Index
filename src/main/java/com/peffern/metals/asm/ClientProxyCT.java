...
public class ClientProxyCT implements IClassTransformer
{
	...
	private byte[] asmify(byte[] bytes)
	{
		...
		for(MethodNode m : classNode.methods)
		{
			if((m.name.equals("registerTileEntities")) && m.desc.equals("(Z)V"))
			{
				...
				while(it.hasNext())
				{
					...
					if(insn instanceof TypeInsnNode)
					{
						...
						{
							TypeInsnNode newTinsn = new TypeInsnNode(tinsn.getOpcode(), "com/peffern/metals/CustomTESRIngotPile");
					...
					else if(insn instanceof MethodInsnNode)
					{
						...
						{
							MethodInsnNode newMinsn = new MethodInsnNode(minsn.getOpcode(), "com/peffern/metals/CustomTESRIngotPile", minsn.name, minsn.desc, minsn.itf);
			...
			else if(m.name.equals("registerRenderInformation") && m.desc.equals("()V"))
			{
				ListIterator<AbstractInsnNode> it = m.instructions.iterator();
				//iterate over the bytecode instructions
				while(it.hasNext())
				{
					...
					{
						TypeInsnNode tinsn = (TypeInsnNode)insn;
						if(tinsn.desc.equals("com/bioxx/tfc/Render/Blocks/RenderMetalTrapDoor"))
						{
							TypeInsnNode newTinsn = new TypeInsnNode(tinsn.getOpcode(), "com/peffern/metals/RenderCustomMetalTrapDoor");
							m.instructions.insert(tinsn,newTinsn);
							m.instructions.remove(tinsn);
						}
					}
					...
					{
						MethodInsnNode minsn = (MethodInsnNode)insn;
						if(minsn.owner.equals("com/bioxx/tfc/Render/Blocks/RenderMetalTrapDoor"))
						{
							MethodInsnNode newMinsn = new MethodInsnNode(minsn.getOpcode(), "com/peffern/metals/RenderCustomMetalTrapDoor", minsn.name, minsn.desc, minsn.itf);
							m.instructions.insert(minsn, newMinsn);
							m.instructions.remove(minsn);
						}
					}
